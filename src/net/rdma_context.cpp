...
int RdmaContext::open_rdma_device(const std::string &device_name, uint8_t port, int gid_index)
{
    int num_devices = 0;
...
int RdmaContext::join_nonblocking_poll_list(int event_fd, int data_fd)
{
    epoll_event event;
                           ...
                           size_t max_cqe)
{
    ...
    if (open_rdma_device(device_name, port, gid_index))
        return -1;
    ...
    if (join_nonblocking_poll_list(event_fd_, context_->async_fd))
        return -1;

    for (size_t i = 0; i < num_comp_channel_; ++i)
        if (join_nonblocking_poll_list(event_fd_, comp_channel_[i]->fd))
            return -1;
    ...
    background_thread_.emplace_back(std::thread(std::bind(&RdmaContext::sender_and_poller, this)));
...
int RdmaContext::register_memory_region(void *addr, size_t length, int access)
{
    if (!ready())
    {
        LOG(ERROR) << "Context for " << device_name() << " not ready";
        return -1;
    }
...
int RdmaContext::unregister_memory_region(void *addr)
{
    if (!ready())
    {
        LOG(ERROR) << "Context for " << device_name() << " not ready";
...
uint32_t RdmaContext::rkey(void *addr)
{
    ...
    LOG(ERROR) << "address " << addr << " rkey not found for " << device_name();
    return 0;
}
...
uint32_t RdmaContext::lkey(void *addr)
{
    ...
    LOG(ERROR) << "address " << addr << " lkey not found for " << device_name();
    return 0;
}
...
int RdmaContext::delete_endpoint(const std::string &remote_server_name)
{
    if (!ready())
    {
        LOG(ERROR) << "Context for " << device_name() << " not ready";
...
ibv_comp_channel *RdmaContext::comp_channel()
{
    if (!ready())
    {
        LOG(ERROR) << "Context for " << device_name() << " not ready";
...
void RdmaContext::sender_and_poller()
{
...
void RdmaContext::sender(int thread_id, int num_threads)
{
    ...
    while (threads_running_)
    {
        ...
        {
            if (endpoint_list[i]->perform_post_send())
                LOG(ERROR) << "Failed to send work requests";
        }
    }
}
