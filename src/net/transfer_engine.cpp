      ...
      vram_buffer_size_(vram_buffer_size)
{
    ...
    int ret = parse_nic_priority_matrix(nic_priority_matrix);
    if (ret)
    {
...
int TransferEngine::join_cluster()
{
    ...
    for (auto &entry : context_list_)
    {
        ...
        device_desc.name = entry->device_name();
    ...
    return metadata_->broadcast_server_desc(local_server_name_, desc);
}

int TransferEngine::leave_cluster()
{
    return metadata_->remove_server_desc(local_server_name_);
}
...
TransferEngine::SegmentID TransferEngine::get_segment_id(const std::string &segment_path)
{
    {
    ...
    if (segment_lookup_map_.count(segment_path))
        return segment_lookup_map_[segment_path];
    auto server_desc = metadata_->get_server_desc(get_server_name(segment_path));
...
int TransferEngine::subscribe_segment(SegmentDesc segment_desc)
{
    LOG(ERROR) << "Not implemented";
    return -1;
}

void *TransferEngine::allocate_local_memory(size_t size, DeviceType type)
{
    void *addr;
    if (type == DeviceType::DRAM)
    {
        addr = allocate_memory_pool(size);
        if (!addr)
            return nullptr;
    }
    else
    {
        LOG(WARNING) << "Not supported device type";
        return nullptr;
    }

    if (register_local_memory(addr, size))
        free_memory_pool(addr, size);

    return addr;
}

int TransferEngine::register_local_memory(void *addr, size_t size)
{
    const static int access_rights = IBV_ACCESS_LOCAL_WRITE;
    for (auto &entry : context_list_)
    {
        int ret = entry->register_memory_region(addr, size, access_rights);
        if (ret)
            return -1;
    }
    return 0;
...
int TransferEngine::unregister_local_memory(void *addr)
{
    for (auto &entry : context_list_)
    {
        int ret = entry->unregister_memory_region(addr);
        if (ret)
            return -1;
    }
    return 0;
...
TransferEngine::BatchID TransferEngine::allocate_batch_id(size_t batch_size)
{
...
int TransferEngine::submit_transfer(BatchID batch_id,
                                    const std::vector<TransferRequest> &entries)
{
    ...
    for (auto &request : entries)
    {
        ...
        if (connect_server(info.server_name))
            return -1;
    }
    ...
    for (auto &entry : slices_to_post)
        entry.first->submit_post_send(entry.second);
    return 0;
}

int TransferEngine::get_transfer_status(BatchID batch_id,
...
int TransferEngine::free_batch_id(BatchID batch_id)
{
...
int TransferEngine::on_setup_rdma_connections(const HandShakeDesc &peer_desc, HandShakeDesc &local_desc)
{
    int device_index = 0;
    local_desc.server_name = local_server_name_;
    auto server_desc = metadata_->get_server_desc(peer_desc.server_name);
    ...
    for (auto &entry : context_list_)
    {
        ...
        local_device_desc.name = entry->device_name();
...
int TransferEngine::parse_nic_priority_matrix(const std::string &nic_priority_matrix)
{
    return metadata_->parse_nic_priority_matrix(nic_priority_matrix, priority_map_, rnic_list_);
}

int TransferEngine::initialize_rdma_resources()
{
    if (rnic_list_.empty())
    {
    ...
    for (auto &rnic : rnic_list_)
    {
        ...
        int ret = context->register_memory_region(dram_buffer_list_[0],
                                                  dram_buffer_size_,
                                                  access_rights);
        if (ret)
        {
...
int TransferEngine::start_handshake_daemon()
{
    return metadata_->start_handshake_daemon(
        std::bind(&TransferEngine::on_setup_rdma_connections,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2));
}

int TransferEngine::connect_server(const std::string &peer_server_name)
{
    {
    ...
    for (auto &context : context_list_)
    {
        ...
        entry.name = context->device_name();
        auto endpoint = context->endpoint(peer_server_name);
        entry.qp_num = endpoint->qp_num();
        local_desc.devices.push_back(entry);
    }

    int rc = metadata_->send_handshake(peer_server_name, local_desc, peer_desc);
    if (rc)
        return rc;
    ...
    for (auto &context : context_list_)
    {
        ...
        entry.name = context->device_name();
        auto endpoint = context->endpoint(peer_server_name);
        rc = endpoint->setup_connection(peer_gid, peer_lid, entry.qp_num);
        if (rc)
            return rc;
        device_index++;
    }
...
int TransferEngine::allocate_internal_buffer()
{
...
int TransferEngine::free_internal_buffer()
{
    for (int socket_id = 0; socket_id < 1; ++socket_id)
...
int TransferEngine::update_rnic_prob(const std::vector<int> &rnic_speed)
{
