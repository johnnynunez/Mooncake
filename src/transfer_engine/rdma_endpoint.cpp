...
namespace mooncake
{
    ...
    int RdmaEndPoint::submitPostSend(std::vector<OldTransferEngine::Slice *> &slice_list,
                                     std::vector<OldTransferEngine::Slice *> &failed_slice_list)
    {
        ...
        for (int i = 0; i < wr_count; ++i)
        {
            ...
            wr.opcode = slice->opcode == OldTransferEngine::TransferRequest::READ ? IBV_WR_RDMA_READ : IBV_WR_RDMA_WRITE;
            wr.num_sge = 1;
            wr.sg_list = &sge;
            ...
            slice->status = OldTransferEngine::Slice::POSTED;
