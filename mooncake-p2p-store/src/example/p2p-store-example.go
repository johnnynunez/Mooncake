...
var (
	...
	nicPriorityMatrix string
	fileSize          int
)

func main() {
	flag.StringVar(&metadataServer, "metadata_server", "http://optane21:2379", "Metadata server address")
	flag.StringVar(&localServerName, "local_server_name", "", "Local server name")
	flag.StringVar(&nicPriorityMatrix, "nic_priority_matrix", "{ \"cpu:0\": [[\"mlx5_2\"], []]}", "NIC priority matrix JSON file")
...
Usage: ./p2p-store-example <trainer|inferencer> 
		                   [--metadata_server=<IP Address>] 
		                   [--local_server_name=<Local Hostname/IP>] 
						   [--nic_priority_matrix=/path/to/json] 
						   [--file_size_mb=<1024-40960>]
		`)
		os.Exit(1)
	}
...
func trainer() {
	...
	store, err := p2pstore.NewP2PStore(metadataServer, localServerName, nicPriorityMatrix)
	if err != nil {
