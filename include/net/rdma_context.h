...
public:
    ...
    int register_memory_region(void *addr, size_t length, int access);

    int unregister_memory_region(void *addr);

    RdmaEndPoint *endpoint(const std::string &remote_server_name);

    int delete_endpoint(const std::string &remote_server_name);
...
public:
    ...
    uint8_t port_num() const { return port_; }
...
private:
    int open_rdma_device(const std::string &device_name, uint8_t port, int gid_index);

    int join_nonblocking_poll_list(int event_fd, int data_fd);

    void sender_and_poller();
