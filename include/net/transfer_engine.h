...
public:
    ...
    // 本地用户内存管理
    //
    // 本地用户内存可以在 Transfer 操作期间被用作读写缓冲区（位于本地）使用，但不可作为数据源（即 Segment）
    // 被其他 CLIENT 利用。

    // 在本地 DRAM/VRAM 上分配长度不小于 size 的空间，并完成 RDMA 注册。
    // - size：分配空间长度；
    // - type: 内存类型，合法值仅为 DRAM 和 VRAM；
    // - 返回值：若成功，返回虚拟地址；否则返回 nullptr。
    void *allocate_local_memory(size_t size, DeviceType type);

    // 在本地 DRAM/VRAM 上注册起始地址为 addr，长度为 size 的空间。
    // - addr: 注册空间起始地址；
    // - size：注册空间长度；
    // - 返回值：若成功，返回 0；否则返回负数值。
    int register_local_memory(void *addr, size_t size);

    // 解注册区域。若该区域是由 allocate_local_memory() 分配的，则同时回收内存空间。
    // - addr: 注册空间起始地址；
    // - 返回值：若成功，返回 0；否则返回负数值。
    int unregister_local_memory(void *addr);
    ...
    BatchID allocate_batch_id(size_t batch_size);
    ...
    // - 返回值：若成功，返回 0；否则返回负数值。
    int submit_transfer(BatchID batch_id,
    ...
    // - 返回值：若成功，返回 0；否则返回负数值。
    int get_transfer_status(BatchID batch_id,
                            std::vector<TransferStatus> &status);

    // 回收 BatchID，之后对此的 submit_transfer 及 get_transfer_status 操作是未定义的。该序号后续可能会被重新使用。
    // - batch_id: 所属的 BatchID ；
    // - 返回值：若成功，返回 0；否则返回负数值。
    int free_batch_id(BatchID batch_id);

    // Helper functions
    void *get_vram_buffer() const { return nullptr; }

    void *get_dram_buffer() const { return dram_buffer_list_[0]; }

    SegmentID get_segment_id(const std::string &segment_path);

    int update_rnic_prob(const std::vector<int> &rnic_speed);
...
public:
    ...
    int subscribe_segment(SegmentDesc segment_desc);
    ...
    int on_setup_rdma_connections(const HandShakeDesc &peer_desc, HandShakeDesc &local_desc);

private:
    int allocate_internal_buffer();

    int free_internal_buffer();

    int parse_nic_priority_matrix(const std::string &nic_priority_matrix);

    int initialize_rdma_resources();

    int start_handshake_daemon();

    int connect_server(const std::string &remote_server_name);
