// transfer_metadata.h
// Copyright (C) 2024 Feng Ren

#ifndef TRANSFER_METADATA
#define TRANSFER_METADATA

#include <atomic>
#include <thread>
#include <string>
#include <memory>
#include <functional>
#include <unordered_map>
#include <glog/logging.h>

#include <jsoncpp/json/json.h>
#include <libmemcached/memcached.hpp>

#include "net/common.h"

// 在 TransferEngine 运行期间需要元数据服务器或外部存储系统维护集群内各节点的可用性及 RDMA
// 参数等， 相关逻辑由被此类抽象，便于与既有系统的集成。 当前基于 memcached/etcd 实现。

/*
    ETCD/MEMCACHED 存放的节点元数据信息

    key = mooncake/[server_name]
    value = {
        'name': 'optane20'
        'devices': [
            { 'name': 'mlx5_2', 'lid': 17, 'gid': 'fe:00:...' },
            { 'name': 'mlx5_3', 'lid': 22, 'gid': 'fe:00:...' }
        ],
        'segments': [
            {
                'name': 'optane20/cpu:0',
                'preferred_rnic': [0],
                'addr': 0x7fa16bdf5000,
                'length': 1073741824,
                'rkey': [1fe000, 1fdf00, ...],
            }
        ],
    }

    节点之间两两互传的 QP_NUM 信息

    value = {
        'server_name': 'optane20',
        'devices': [
            { 'name': 'mlx5_2', 'qp_num': [xxx, yyy] },
            { 'name': 'mlx5_3', 'qp_num': [xxx, yyy] },
        ]
    }
*/

const static uint16_t kDefaultServerPort = 12001;

struct TransferMetadataImpl;

class TransferMetadata
{
public:
    struct DeviceDesc
    {
        std::string name;
        uint16_t lid;
        std::string gid;
    };

    struct SegmentDesc
    {
        std::string name;
        uint64_t addr;
        uint64_t length;
        std::vector<uint32_t> rkey;
        std::vector<int> preferred_rnic;
    };

    struct ServerDesc
    {
        std::string name;
        std::vector<DeviceDesc> devices;
        std::vector<SegmentDesc> segments;
    };

    struct HandShakeDescImpl
    {
        std::string name;
        std::vector<uint32_t> qp_num;
    };

    struct HandShakeDesc
    {
        std::string server_name;
        std::vector<HandShakeDescImpl> devices;
    };

public:
    TransferMetadata(const std::string &metadata_uri);

    ~TransferMetadata();

    int broadcast_server_desc(const std::string &server_name, const ServerDesc &desc);

    std::shared_ptr<ServerDesc> get_server_desc(const std::string &server_name);

    int remove_server_desc(const std::string &server_name);

    using OnReceiveHandShake = std::function<int(const HandShakeDesc &peer_desc, HandShakeDesc &local_desc)>;
    int start_handshake_daemon(OnReceiveHandShake on_receive_handshake,
                               uint16_t listen_port = kDefaultServerPort);

    int send_handshake(const std::string &peer_server_name,
                       const HandShakeDesc &local_desc,
                       HandShakeDesc &peer_desc);

    struct PriorityItem
    {
        std::vector<std::string> preferred_rnic_list;
        std::vector<std::string> available_rnic_list;
    };
    using PriorityMap = std::unordered_map<std::string, PriorityItem>;
    static int parse_nic_priority_matrix(const std::string &nic_priority_matrix,
                                         PriorityMap &priority_map,
                                         std::vector<std::string> &rnic_list);

private:
    std::string encode(const HandShakeDesc &desc);

    int decode(const std::string &ser, HandShakeDesc &desc);

private:
    std::atomic<bool> listener_running_;
    std::thread listener_;
    OnReceiveHandShake on_receive_handshake_;

    std::shared_ptr<TransferMetadataImpl> impl_;

    RWSpinlock server_desc_lock_;
    std::unordered_map<std::string, std::shared_ptr<ServerDesc>> server_desc_map_;
};

#endif // TRANSFER_METADATA