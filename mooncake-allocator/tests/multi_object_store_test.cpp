...
char randomChar() { return static_cast<char>(dis(gen)); }
   ...
   protected:
    ...
    void TearDown() override {
        for (auto &meta : segment_and_index) {
            LOG(INFO) << "meta info, segment_id: " << meta.first
                      << ", buffer size: " << meta.second.size();
    ...
    uint64_t testRegisterBuffer(DistributedObjectStore &store,
        ...
        uint64_t index = store.registerBuffer(
            segmentId, reinterpret_cast<size_t>(ptr), size);
        EXPECT_GE(index, 0);
        return index;
    }
...
TEST_F(DistributedObjectStoreMultiThreadTest, ConcurrentReplicateAndGetTest) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back([this, &keys, &data, &configs, &versions, i]() {
            ...
            TaskID replicateVersion =
                store.replicate(keys[i], newConfig, replicaDiff);
...
TEST_F(DistributedObjectStoreMultiThreadTest, ConcurrentMixedOperationsTest) {
    ...
    for (int i = 0; i < numKeys; ++i) {
        std::vector<Slice> putSlices = {
            {initialData[i].data(), initialData[i].size()}};
    ...
    for (int i = 0; i < numThreads; ++i) {
        threads.emplace_back([this, &keys, &initialData, &configs,
                              &initialVersions, &start, &gen, &totalOperations,
                              &sizeDist, &replicaDist, numKeys,
                              numOperationsPerThread]() {
            std::uniform_int_distribution<> keyDist(0, numKeys - 1);
            std::uniform_int_distribution<> opDist(
                0, 3);  // 0: put, 1: get, 2: remove, 3: replicate
                ...
                try {
                    switch (operation) {
                        case 0:  // put
                        {
                            ...
                            std::generate(newData.begin(), newData.end(),
                                          randomChar);
                            std::vector<Slice> putSlices = {
                                {newData.data(), newData.size()}};
                            TaskID version = store.put(
                                keys[keyIndex], putSlices, configs[keyIndex]);
                            EXPECT_NE(version, 0);
                            break;
                        }
                        case 1:  // get
                        {
                            std::vector<char> retrievedData(
                                initialData[keyIndex].size());
                            std::vector<Slice> getSlices = {
                                {retrievedData.data(), retrievedData.size()}};
                            TaskID getVersion =
                                store.get(keys[keyIndex], getSlices, 0,
                                          0);  // Get latest version
                            if (getVersion < 0) {
                                LOG(ERROR)
                                    << "get key: " << keys[keyIndex]
                                    << " , ret: " << errnoToString(getVersion);
                            }
                            break;
                        }
                        case 2:  // remove
                        {
                            TaskID removeVersion = store.remove(
                                keys[keyIndex]);  // Remove latest version
                            if (removeVersion < 0) {
                                LOG(ERROR)
                                    << "remove key: " << keys[keyIndex]
                                    << ", ret:" << errnoToString(removeVersion);
                            }
                            break;
                        }
                        case 3:  // replicate
                        {
                            ...
                            DistributedObjectStore::ReplicaDiff replicaDiff;
                            TaskID replicateVersion = store.replicate(
                                keys[keyIndex], newConfig, replicaDiff);
                            if (replicateVersion < 0) {
                                LOG(ERROR) << "replica key: " << keys[keyIndex]
                                           << ", ret: "
                                           << errnoToString(replicateVersion);
                            }
                            break;
                        }
                    }
    ...
    for (int i = 0; i < numKeys; ++i) {
        ...
        TaskID finalVersion =
            store.get(keys[i], getSlices, 0, 0);  // Get latest version
