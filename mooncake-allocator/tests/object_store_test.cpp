...
class DistributedObjectStoreTest : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        for (int i = 0; i < 10; i++) {
            for (SegmentID segment_id = 1; segment_id <= 6; segment_id++) {
                segment_and_index[segment_id].push_back(
                    testRegisterBuffer(store, segment_id));
            }
    ...
    uint64_t testRegisterBuffer(DistributedObjectStore &store,
                                SegmentId segmentId) {
    ...
    void testUnregisterBuffer(DistributedObjectStore &store,
                              SegmentId segmentId, uint64_t index) {
...
TEST_F(DistributedObjectStoreTest, PutGetTest) {
    ...
    std::vector<Slice> getSlices = {
        {retrievedData.data(), retrievedData.size()}};
...
void CompareAndLog(const std::vector<char> &combinedPutData,
                   const std::vector<char> &combinedGetData, size_t offset,
                   size_t compareSize) {
    if (memcmp(combinedPutData.data() + offset, combinedGetData.data(),
               compareSize) != 0) {
        LOG(ERROR) << "Comparison failed: memcmp(combinedPutData.data() + "
                   << offset << ", combinedGetData.data(), " << compareSize
                   << ") != 0";
        LOG(ERROR) << "combinedPutData size: "
                   << combinedPutData.size() - offset << ", content: "
                   << std::string(combinedPutData.data() + offset, compareSize);
        LOG(ERROR) << "combinedGetData size: " << combinedGetData.size()
                   << ",content: "
                   << std::string(combinedGetData.data(), compareSize);
    }
}
...
TEST_F(DistributedObjectStoreTest, RandomSizePutGetTest) {
    ...
    for (int testIteration = 0; testIteration < 10; ++testIteration) {
        ...
        ObjectKey key =
            "random_size_test_object_" + std::to_string(testIteration);
        TaskID putVersion = store.put(key, slices, config);
        EXPECT_NE(putVersion, 0);
        ...
        for (int i = 0; i < sliceCount; ++i) {
            ...
            EXPECT_EQ(
                memcmp(data[i].data(), retrievedData[i].data(), data[i].size()),
                0);
        }
    }
}
...
TEST_F(DistributedObjectStoreTest, RandomSizeOffsetPutGetTest) {
    ...
    for (int testIteration = 0; testIteration < 10; ++testIteration) {
        ...
        ObjectKey key =
            "random_size_offset_test_object_" + std::to_string(testIteration);
        ...
        LOG(INFO) << "the putdata, size: " << compareSize
                  << " , content: " << putdata;
        LOG(INFO) << "the getdata, size: " << compareSize
                  << " , content: " << getdata;
        CompareAndLog(combinedPutData, combinedGetData, offset, compareSize);

        EXPECT_EQ(memcmp(combinedPutData.data() + offset,
                         combinedGetData.data(), compareSize),
                  0);
    }
}
...
TEST_F(DistributedObjectStoreTest, OverwriteExistingKeyTest) {
    ...
    std::vector<Slice> initialSlices = {
    ...
    std::vector<Slice> getInitialSlices = {
        {retrievedInitialData.data(), retrievedInitialData.size()}};
    TaskID getInitialVersion = store.get(key, getInitialSlices, 0, 0);
    EXPECT_EQ(getInitialVersion, initialVersion);
    EXPECT_EQ(memcmp(initialData.data(), retrievedInitialData.data(),
                     initialData.size()),
              0);
    ...
    std::vector<Slice> getNewSlices = {
        {retrievedNewData.data(), retrievedNewData.size()}};
    TaskID getNewVersion = store.get(key, getNewSlices, newVersion, 0);
    EXPECT_EQ(getNewVersion, newVersion);
    EXPECT_EQ(memcmp(newData.data(), retrievedNewData.data(), newData.size()),
              0);

    EXPECT_NE(memcmp(retrievedInitialData.data(), retrievedNewData.data(),
                     retrievedInitialData.size()),
              0);
}
...
TEST_F(DistributedObjectStoreTest, RemoveAndPutTest) {
    ...
    std::vector<Slice> getRemovedSlices = {
        {retrievedRemovedData.data(), retrievedRemovedData.size()}};
...
TEST_F(DistributedObjectStoreTest, CheckAllTest) { store.checkAll(); }
