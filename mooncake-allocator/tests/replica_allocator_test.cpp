...
void testAddOneReplica(std::shared_ptr<ReplicaAllocator> &allocator) {
    std::cout
        << "-----------------testAddOneReplica begin----------------------\n";
    ObjectKey key = "test_object";
    ReplicaInfo info;
    Version version =
        allocator->addOneReplica(key, info, DEFAULT_VALUE, 2048);  // 2KB object
    LOG(INFO) << "testAddOneReplica, the version: " << version;
    if (info.handles.size() == 2 && info.handles[0]->size == 1024 &&
        info.handles[1]->size == 1024) {
        std::cout << "-----------------testAddOneReplica "
                     "passed----------------------\n";
    } else {
        std::cout << "------------------testAddOneReplica "
                     "failed-------------------------\n";
...
void testAddMultipleReplicas(std::shared_ptr<ReplicaAllocator> &allocator) {
    std::cout << "----------------------------testAddMultipleReplicas "
                 "begin---------------------------\n";

    ObjectKey key = "multi_replica_object";
    ReplicaInfo info1, info2, info3;

    Version v1 = allocator->addOneReplica(key, info1, DEFAULT_VALUE,
                                          3072);  // 3KB object
    Version v2 = allocator->addOneReplica(key, info2, v1);
    Version v3 = allocator->addOneReplica(key, info3, v1);
    LOG(INFO) << "testAddMultipleReplicas, the version: " << v1 << " " << v2
              << " " << v3;
    if (v1 == v2 && v2 == v3 && info1.handles.size() == 3 &&
        info2.handles.size() == 3 && info3.handles.size() == 3) {
        std::cout << "----------------------------testAddMultipleReplicas "
                     "passed---------------------------\n";
    } else {
        std::cout << "-------------------------testAddMultipleReplicas "
...
void testReassignReplica(std::shared_ptr<ReplicaAllocator> &allocator) {
    ...
    if (new_info.handles.size() == 2 &&
        new_info.handles[0] != info.handles[0] &&
...
void testLargeObjectAllocation(std::shared_ptr<ReplicaAllocator> &allocator) {
    std::cout << "-----------------------testLargeObjectAllocation "
                 "begin----------------------\n";
    ObjectKey key = "large_object";
    ReplicaInfo info;
    Version version = allocator->addOneReplica(key, info, DEFAULT_VALUE,
                                               20480);  // 20KB object
...
void testRecovery(std::shared_ptr<ReplicaAllocator> &allocator,
                  SegmentId segment, uint64_t allocator_index) {
    std::vector<std::shared_ptr<BufHandle>> handles =
        allocator->unregister(segment, allocator_index);
...
void concurrentTest(std::shared_ptr<ReplicaAllocator> allocator, int threadId,
                    std::atomic<bool> &stop) {
    // while (!stop.load(std::memory_order_relaxed)) {
    try {
        runAllTests(allocator);
        std::cout << "Thread " << threadId << " completed one round of tests."
                  << std::endl;
    } catch (const std::exception &e) {
        std::cerr << "Thread " << threadId << ", stop: " << stop
