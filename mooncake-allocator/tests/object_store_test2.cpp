...
TEST_F(DistributedObjectStoreGenerateTest, GenerateWriteTransferRequests) {
    ...
    std::vector<void *> sizes = {reinterpret_cast<void *>(512),
                                 reinterpret_cast<void *>(1024)};
    std::vector<TransferRequest> transfer_tasks;

    // 创建一些模拟的 BufHandle
    std::shared_ptr<BufHandle> handle1 =
        std::make_shared<BufHandle>(nullptr, 1, 512, malloc(512));
    std::shared_ptr<BufHandle> handle2 =
        std::make_shared<BufHandle>(nullptr, 2, 1024, malloc(1024));
    replica_info.handles = {handle1, handle2};

    store->generateWriteTransferRequests(replica_info, ptrs, sizes,
                                         transfer_tasks);
...
TEST_F(DistributedObjectStoreGenerateTest, GenerateReadTransferRequests) {
    ...
    store->generateReadTransferRequests(handles, 0, ptrs, sizes,
                                        transfer_tasks);
...
TEST_F(DistributedObjectStoreGenerateTest, GenerateReplicaTransferRequests) {
    ...
    std::shared_ptr<BufHandle> new_handle1 =
        std::make_shared<BufHandle>(nullptr, 3, 512, malloc(512));
    std::shared_ptr<BufHandle> new_handle2 =
        std::make_shared<BufHandle>(nullptr, 4, 1024, malloc(1024));
    new_replica_info.handles = {new_handle1, new_handle2};

    store->generateReplicaTransferRequests(existed_replica_info,
