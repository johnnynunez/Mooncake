...
DistributedObjectStore::DistributedObjectStore() :
        replica_allocator_(1024 * 64), allocation_strategy_(nullptr), max_trynum_(10) {
...
TaskID DistributedObjectStore::replicate(ObjectKey key, ReplicateConfig new_config, ReplicaDiff& replica_diff) {
    ...
    // 比较新老config
    if (new_config.replica_num > existed_replica_number) {  // need add
    ...
    } else if (new_config.replica_num < existed_replica_number) {  // need remove
    ...
    const std::vector<TransferRequest>& transfer_tasks)
{
    ...
    for (int task_id = 0; task_id < transfer_tasks.size(); ++task_id) {
        ...
        if (input_offset == size_values[input_idx] || task_id == transfer_tasks.size() - 1) {
            LOG(INFO) << "enter if: " << "before_input_idx: " << input_idx << ", input_offset: " << input_offset << ", size_values[input_idx]: " << size_values[input_idx] << ", task_id: " << task_id << ", transfer_tasks.size(): " << transfer_tasks.size();
            input_idx++;
            input_offset = 0;
        }
