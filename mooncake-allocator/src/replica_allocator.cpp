...
ReplicaAllocator::ReplicaAllocator(size_t shard_size)
    : shard_size_(shard_size), global_version_(0) {  // version从1开始
...
size_t ReplicaAllocator::getShardSize() { return shard_size_; }
uint64_t ReplicaAllocator::registerBuffer(SegmentId segment_id, size_t base,
                                          size_t size) {
    std::unique_lock<std::shared_mutex> lock(
        buf_allocators_mutex_);  // write lock
    buf_allocators_[segment_id].emplace_back(
        std::make_shared<BufferAllocator>(segment_id, base, size));

    LOG(INFO) << "Registered buffer for segment " << segment_id << " with base "
              << (void *)base << " and size " << size;
    return buf_allocators_[segment_id].size() - 1;
}

Version ReplicaAllocator::addOneReplica(
    const ObjectKey &key, ReplicaInfo &ret, Version ver, uint64_t object_size,
    std::shared_ptr<AllocationStrategy> strategy) {
    ret.reset();
    if (ver == DEFAULT_VALUE && object_size == DEFAULT_VALUE) {
        LOG(ERROR) << "Invalid arguments: ver and object_size cannot both be "
                      "DEFAULT_VALUE";
        throw std::invalid_argument(
            "ver and object_size cannot both be DEFAULT_VALUE");
    }

    std::unique_lock<std::shared_mutex> lock(object_meta_mutex_);  // write lock
    if (object_meta_.count(key) == 0) {  // Key does not exist
        ver = DEFAULT_VALUE;
    }
    ...
    if (object_size != DEFAULT_VALUE && ver == DEFAULT_VALUE) {
        size = object_size;
    } else {  // If both ver and object_size are specified, use ver information
        if (replicas.size() == 0) {
            LOG(ERROR) << "no completed replica for addonereplica, key: " << key
                       << " ,version: " << ver;
    ...
    LOG(INFO) << "Adding replica for key " << key << ", version "
              << target_version << ", size " << size << ", num_shards "
              << num_shards;
    ...
    for (size_t i = 0; i < num_shards; ++i) {
        ...
        if (i == num_shards - 1) {
            ...
            if (shard_size == 0) {
                shard_size = shard_size_;  // If the last shard size is 0, set
                                           // it to shard_size_
            }
        } else {
        ...
        while (!allocated) {
            ...
            if (try_num > max_select_num_) {
                LOG(ERROR)
                    << "Can't select good segment for addreplica after trying";
                return getError(ERRNO::AVAILABLE_SEGMENT_EMPTY);
            }
            SegmentId segment_id = strategy->selectSegment(
                buf_allocators_, replicas, i, failed_segment_ids);
            if (buf_allocators_.count(segment_id) == 0) {
                LOG(WARNING) << "Selected segment " << segment_id
                             << " not found in buf_allocators_";
                continue;
            }
            ...
            for (size_t j = 0; j < buf_allocators_[segment_id].size(); ++j) {
                ...
                if (handle) {
                    ...
                    handles_[segment_id][j].push_back(
                        handle);  // For global handle indexing
                    strategy->selected(segment_id, j, shard_size);
                    allocated = true;
                    LOG(INFO) << "Allocated shard " << i << " in segment "
                              << segment_id << ", allocator " << j;
                    break;
                } else {
                    LOG(WARNING)
                        << "Failed to allocate shard " << i << " in segment "
                        << segment_id << ", allocator " << j;
                }
            }
        }
    ...
    if (ret.handles.size() != num_shards) {
        LOG(ERROR) << "Cannot allocate needed handle, need : " << num_shards
                   << " , allocate: " << ret.handles.size();
    ...
    LOG(INFO) << "Added replica for key " << key << ", version "
              << target_version << ", replica_id: " << replica_id;
    return target_version;
}

Version ReplicaAllocator::getOneReplica(
    const ObjectKey &key, ReplicaInfo &ret, Version ver,
    std::shared_ptr<AllocationStrategy> strategy) {
    ...
    if (target_version < ver) {
        LOG(ERROR) << "Invalid version " << ver << ", current version is "
                   << target_version;
    ...
    if (version_list.versions.count(target_version) == 0 ||
        ...
        LOG(ERROR) << "No replica found for key " << key << ", version "
                   << target_version;
        ret.status = ReplicaStatus::FAILED;
        return getError(ERRNO::INVALID_VERSION);
    }
    ...
    for (size_t i = 0; i < handles_per_replica; ++i) {
        try {
            auto handle = strategy->selectHandle(replicas, current_handle_index,
                                                 failed_bufhandle);
            if (handle == nullptr) {
                throw std::runtime_error(
                    "Unable to select handle even after reset");
            }
            ret.handles.push_back(handle);
            ret.replica_id = handle->replica_meta
                                 .replica_id;  // Temporarily take the
                                               // replica_id of the first handle
    ...
    LOG(INFO) << "get one replica, key " << key << ", version "
              << target_version << ", replica_id " << ret.replica_id
              << ", handles " << ret.handles.size() << ", status "
              << (int)ret.status;
    return target_version;
}

void ReplicaAllocator::reassignReplica(const ObjectKey &key, Version ver,
                                       int replica_id, ReplicaInfo &ret) {
    std::unique_lock<std::shared_mutex> lock(object_meta_mutex_);  // write lock
    auto &version_list = object_meta_[key];
    auto &version_info = version_list.versions[ver];
    auto &old_replica = version_info.replicas[replica_id];

    LOG(INFO) << "Reassigning replica for key " << key << ", version " << ver
    ...
    for (size_t i = 0; i < old_replica.handles.size(); ++i) {
        if (old_replica.handles[i]->status == BufStatus::FAILED) {
            ...
            auto new_handle =
                allocateShard(DEFAULT_VALUE, DEFAULT_VALUE, shard_size);
            ret.handles.push_back(new_handle);
            LOG(INFO) << "Reallocated shard " << i << " with size "
                      << shard_size;
        } else {
    ...
    LOG(INFO) << "Completed reassignment of replica for key " << key
              << ", version " << ver << ", replica_id " << replica_id;
}

Version ReplicaAllocator::removeOneReplica(const ObjectKey &key,
                                           ReplicaInfo &ret, Version ver) {
    std::unique_lock<std::shared_mutex> lock(object_meta_mutex_);  // write lock
    auto &version_list = object_meta_[key];
    Version target_version =
        (ver != DEFAULT_VALUE) ? ver : version_list.flushed_version;
    ...
    if (!replicas.empty()) {
        ...
        LOG(INFO) << "Removed replica for key " << key << ", version "
                  << target_version;
    } else {
        LOG(WARNING) << "No replicas to remove for key " << key << ", version "
                     << target_version;
        return getError(ERRNO::INVALID_VERSION);
    }
    return target_version;
...
std::vector<std::shared_ptr<BufHandle>> ReplicaAllocator::unregister(
    SegmentId segment_id, uint64_t buffer_index) {
    std::unique_lock<std::shared_mutex> buf_lock(
        buf_allocators_mutex_);  // write lock

    // Unregister the corresponding bufhandle in the replica and release the
    // bufallocator
    std::vector<std::shared_ptr<BufHandle>> handles;
    if (buf_allocators_.count(segment_id) == 0 ||
        buffer_index >= buf_allocators_[segment_id].size()) {
        LOG(WARNING) << "Failed to unregister buffer for segment " << segment_id
    ...
    buf_allocators_[segment_id].erase(buf_allocators_[segment_id].begin() +
                                      buffer_index);

    LOG(INFO) << "Unregistered buffer for segment " << segment_id
              << ", buffer index " << buffer_index;
    return handles;
}
...
size_t ReplicaAllocator::recovery(
    ...
    for (auto &old_handle : old_handles) {
        ...
        if (object_meta_.count(replica_meta.object_name) == 0 ||
            object_meta_[replica_meta.object_name].versions.count(
                replica_meta.version) == 0) {
            LOG(ERROR) << "Invalid object meta, object_name: "
                       << replica_meta.object_name
                       << ", version: " << replica_meta.version;
            continue;
        }
        auto &version_info = object_meta_[replica_meta.object_name]
                                 .versions[replica_meta.version];
        ...
        while (!allocated && try_num++ < max_select_num_) {
            SegmentId segment_id = strategy->selectSegment(
                buf_allocators_, version_info.replicas, replica_meta.shard_id,
                failed_segment_ids);
            if (buf_allocators_.count(segment_id) == 0) {
                LOG(WARNING) << "Selected segment " << segment_id
                             << " not found in buf_allocators_";
            ...
            for (size_t j = 0; j < buf_allocators_[segment_id].size(); ++j) {
                auto new_handle =
                    allocateShard(segment_id, j, old_handle->size);
                if (new_handle) {
                    ...
                    allocated = true;
                    new_handles_num++;
                    LOG(INFO) << "Allocated shard in segment " << segment_id
                              << ", allocator " << j;
                    break;
                } else {
                    LOG(WARNING)
                        << "Allocate shard failed, segment_id: " << segment_id
                        << ", index:" << j;
                }
            }
        }
        if (!allocated) {
            LOG(ERROR) << "Failed to allocate shard for recovery after "
                       << try_num << " attempts.";
        }
    }
    LOG(INFO) << "recovery handles num: " << old_handles.size()
              << ", new handles num: " << new_handles_num;
...
void ReplicaAllocator::updateStatus(const ObjectKey &key, ReplicaStatus status,
                                    size_t index, Version ver) {
    std::unique_lock<std::shared_mutex> lock(object_meta_mutex_);  // write lock
    if (object_meta_.count(key) == 0) {
...
std::vector<ReplicaStatus> ReplicaAllocator::getStatus(const ObjectKey &key,
                                                       Version ver) {
...
std::shared_ptr<BufHandle> ReplicaAllocator::allocateShard(
    SegmentId segment_id, uint64_t allocator_index, size_t size) {
    ...
    if (segment_id == DEFAULT_VALUE || allocator_index == DEFAULT_VALUE) {
        ...
        for (const auto &[sid, allocators] : buf_allocators_) {
            for (size_t i = 0; i < allocators.size(); ++i) {
                if ((handle = allocators[i]->allocate(size)) != nullptr) {
                    LOG(INFO) << "Allocated shard of size " << size
                              << " in segment " << sid << ", allocator " << i;
                    return handle;
                }
            }
        }
        ...
        if (handle) {
            LOG(INFO) << "Allocated shard of size " << size << " in segment "
                      << segment_id << ", allocator " << allocator_index;
        } else {
            LOG(WARNING) << "Failed to allocate shard of size " << size
                         << " in segment " << segment_id << ", allocator "
                         << allocator_index;
        }
        return handle;
    }
...
size_t ReplicaAllocator::getReplicaRealNumber(const ObjectKey &key,
                                              Version version) {
    if (object_meta_.count(key) == 0) {
        LOG(WARNING) << "no key: " << key << " existed, version:" << version
                     << " when getReplicaRealNumber";
        return 0;
    }
    if (object_meta_[key].versions.count(version) == 0) {
        LOG(WARNING) << "key: " << key << " no existed version:" << version
                     << " when getReplicaRealNumber";
        return 0;
    }
...
size_t ReplicaAllocator::cleanUncompleteReplica(const ObjectKey &key,
                                                Version version,
                                                int max_replica_num) {
    ...
    if (max_replica_num < real_replica_num) {
        LOG(WARNING) << "max_replica_num is over real_replica_num, shouldn't "
                        "happen, max_replica_num: "
                     << max_replica_num
                     << ", real_replica_num: " << real_replica_num;
        return 0;
    }

    int keep_partial_num = max_replica_num - real_replica_num;

    for (auto it = version_info.replicas.begin();
         it != version_info.replicas.end();) {
        ...
        if (replica.status == ReplicaStatus::PARTIAL && keep_partial_num > 0) {
            ...
            LOG(INFO) << "Removing partial replica: key: " << key
                      << " version: " << version
                      << " replica_id: " << it->first;
            it = version_info.replicas.erase(it);
            cleanNum++;
        } else {
            LOG(INFO) << "Clean uncomplete replica, the key: " << key
                      << " version: " << version << " replica_id: " << it->first
                      << " status: " << static_cast<uint64_t>(replica.status);
            ++it;
        }
    }
    return cleanNum;
