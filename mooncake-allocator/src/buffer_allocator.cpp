...
BufHandle::BufHandle(std::shared_ptr<BufferAllocator> allocator, int segment_id, uint64_t size, void* buffer) :
        allocator_(allocator), segment_id(segment_id), size(size), status(BufStatus::INIT), buffer(buffer) {
