...
BufHandle::BufHandle(std::shared_ptr<BufferAllocator> allocator, int segment_id,
    ...
    LOG(INFO) << "Created BufHandle for segment " << segment_id << ", size "
              << size << ", buffer address: " << buffer;
}
...
BufHandle::~BufHandle() {
    if (allocator_) {
        ...
        LOG(INFO) << "Deallocated BufHandle for segment " << segment_id
                  << ", size " << size;
    } else {
...
BufferAllocator::BufferAllocator(int segment_id, size_t base, size_t size)
    : segment_id_(segment_id), base_(base), total_size_(size) {
    LOG(INFO) << "Initializing BufferAllocator for segment " << segment_id
              << ", base " << (void *)base << ", size " << size;
...
std::shared_ptr<BufHandle> BufferAllocator::allocate(size_t size) {
    ...
    LOG(INFO) << "Allocated " << size << " bytes in segment " << segment_id_
              << ", the address: " << buffer;
    // 创建并返回新的 BufHandle
    auto bufHandle = std::make_shared<BufHandle>(shared_from_this(),
                                                 segment_id_, size, buffer);
    return bufHandle;
}
void BufferAllocator::deallocate(BufHandle *handle) {
    try {
        ...
        LOG(INFO) << "Deallocated address: " << handle->buffer << " , "
                  << handle->size << " bytes in segment " << segment_id_;
    } catch (const std::exception &e) {
        // 捕获所有标准异常并打印异常信息
