...
SegmentId RandomAllocationStrategy::selectSegment(
    ...
    const int shard_index, std::vector<SegmentId> &failed_segment_ids) {
    ...
    for (const auto &[replica_id, replica] : replica_list) {
        if (shard_index >= (int)replica.handles.size()) {
            LOG(ERROR) << "wrong shard_index: " << shard_index
                       << " in selectSegment";
...
void RandomAllocationStrategy::selected(SegmentId segment_id, int buf_index,
                                        size_t size) {
    // Implement if needed
    LOG(INFO) << "selected segment_id: " << segment_id
              << ", buf_index: " << buf_index << ", size: " << size;
    return;
}
...
std::shared_ptr<BufHandle> RandomAllocationStrategy::selectHandle(
    ...
    LOG(ERROR) << "cannot select right handle, current_handle_index: "
               << current_handle_index
               << ", the failed_bufhandle size: " << failed_bufhandle.size();
    return nullptr;
}

void RandomAllocationStrategy::updateConfig(
    const std::shared_ptr<AllocationStrategyConfig> &new_config) {
    auto cast_config =
        std::dynamic_pointer_cast<RandomAllocationStrategyConfig>(new_config);
    if (!cast_config) {
