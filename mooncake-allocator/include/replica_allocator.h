...
class ReplicaAllocator {
   public:
    ...
    Version addOneReplica(
        const ObjectKey &key, ReplicaInfo &ret, Version ver = -1,
    ...
    void reassignReplica(const ObjectKey &key, Version ver, int replica_id,
                         ReplicaInfo &ret);

    // Removes a replica for the given object key.
    // Returns the version of the removed replica.
    Version removeOneReplica(const ObjectKey &key, ReplicaInfo &ret,
                             Version ver = -1);

    // Unregisters the buffer allocator for the given segment ID and buffer
    // index. Returns the list of buffer handles that were unregistered.
    std::vector<std::shared_ptr<BufHandle>> unregister(SegmentId segment_id,
                                                       uint64_t buffer_index);

    // Recovers the given buffer handles by reallocating their space.
    // Returns the number of successfully reallocated handles.
    size_t recovery(std::vector<std::shared_ptr<BufHandle>> &old_handles,
                    std::shared_ptr<AllocationStrategy> strategy = nullptr);
    ...
    // Retrieves the number of real replicas for the given object key and
    // version.
    size_t getReplicaRealNumber(const ObjectKey &key, Version version);

    // Cleans up incomplete replicas and ensures that at least `max_replica_num`
    // complete replicas are retained. If the number of complete replicas is
    // less than `max_replica_num`, partial replicas are also retained.
    size_t cleanUncompleteReplica(const ObjectKey &key, Version version,
                                  int max_replica_num);

    // Updates the status of the replica for the given object key.
    // The index specifies which replica to update, defaulting to the latest
    // replica.
    void updateStatus(const ObjectKey &key, ReplicaStatus status,
                      size_t index = -1, Version ver = -1);

    // Gets the status of the replica for the given object key.
    std::vector<ReplicaStatus> getStatus(const ObjectKey &key,
                                         Version ver = -1);
   ...
   private:
    ...
    std::shared_ptr<BufHandle> allocateShard(SegmentId segment_id,
   ...
   private:
    ...
    std::map<SegmentId, std::vector<std::shared_ptr<BufferAllocator>>>
        buf_allocators_;
    std::map<SegmentId,
             std::map<uint64_t, std::vector<std::weak_ptr<BufHandle>>>>
        handles_;
