...
#define XLOG(level, ...)                                                     \
    XLOG_IMPL(::folly::LogLevel::level, ::folly::LogStreamProcessor::APPEND, \
              ##__VA_ARGS__)
...
#define XLOG_IF(level, cond, ...)                \
    XLOG_IF_IMPL(::folly::LogLevel::level, cond, \
                 ::folly::LogStreamProcessor::APPEND, ##__VA_ARGS__)
/**
 * Log a message to this file's default log category, using a format string.
 */
#define XLOGF(level, fmt, ...)                                               \
    XLOG_IMPL(::folly::LogLevel::level, ::folly::LogStreamProcessor::FORMAT, \
              fmt, ##__VA_ARGS__)
...
#define XLOGF_IF(level, cond, fmt, ...)          \
    XLOG_IF_IMPL(::folly::LogLevel::level, cond, \
                 ::folly::LogStreamProcessor::FORMAT, fmt, ##__VA_ARGS__)
...
#define XLOG_EVERY_MS_IF(level, cond, ms, ...)                                 \
    XLOG_IF(                                                                   \
        ...
        (cond) &&                                                              \
            [__folly_detail_xlog_ms = ms] {                                    \
                static ::folly::logging::IntervalRateLimiter                   \
                    folly_detail_xlog_limiter(1, ::std::chrono::milliseconds(  \
                                                     __folly_detail_xlog_ms)); \
                return folly_detail_xlog_limiter.check();                      \
            }(),                                                               \
        ##__VA_ARGS__)
...
#define XLOG_EVERY_N_IF(level, cond, n, ...)                                   \
    XLOG_IF(                                                                   \
        ...
        (cond) &&                                                              \
            [&] {                                                              \
                struct folly_detail_xlog_tag {};                               \
                return ::folly::detail::xlogEveryNImpl<folly_detail_xlog_tag>( \
                    n);                                                        \
            }(),                                                               \
        ##__VA_ARGS__)
...
#define XLOG_EVERY_N_EXACT(level, n, ...)                \
    XLOG_IF(                                             \
        ...
        [&] {                                            \
            struct folly_detail_xlog_tag {};             \
            return ::folly::detail::xlogEveryNExactImpl< \
                folly_detail_xlog_tag>(n);               \
        }(),                                             \
        ##__VA_ARGS__)
...
#define XLOG_EVERY_N_THREAD(level, n, ...)                \
    XLOG_IF(                                              \
        ...
        [&] {                                             \
            struct folly_detail_xlog_tag {};              \
            return ::folly::detail::xlogEveryNThreadImpl< \
                folly_detail_xlog_tag>(n);                \
        }(),                                              \
        ##__VA_ARGS__)
...
#define XLOG_N_PER_MS(level, count, ms, ...)                                \
    XLOG_IF(                                                                \
        ...
        [] {                                                                \
            static ::folly::logging::IntervalRateLimiter                    \
                folly_detail_xlog_limiter((count),                          \
...
#define XLOG_FIRST_N(level, n, ...)                      \
    XLOG_IF(                                             \
        ...
        [&] {                                            \
            struct folly_detail_xlog_tag {};             \
            return ::folly::detail::xlogFirstNExactImpl< \
                folly_detail_xlog_tag>(n);               \
        }(),                                             \
        ##__VA_ARGS__)
...
#define XLOG_IMPL(level, type, ...)                                      \
    XLOG_ACTUAL_IMPL(level, true, ::folly::isLogLevelFatal(level), type, \
                     ##__VA_ARGS__)
...
#define XLOG_IS_ON_IMPL_HELPER(level)                               \
    ([] {                                                           \
        ...
        return folly_detail_xlog_level.check(                       \
            (level), folly_detail_xlog_category,                    \
...
#define XLOG_SET_CATEGORY_NAME(category)                                       \
    ...
    static_assert(                                                             \
        ::std::is_same<xlog_correct_usage,                                     \
 ...
 */
#define XCHECK(cond, ...)                                               \
    XLOG_IF(FATAL, FOLLY_UNLIKELY(!(cond)), "Check failed: " #cond " ", \
            ##__VA_ARGS__)
...
#define XDCHECK_OP(op, arg1, arg2, ...) \
    while (false) XCHECK_OP(op, arg1, arg2, ##__VA_ARGS__)
#else
