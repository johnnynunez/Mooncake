...
class SlabAllocator {
   public:
    // See Feishu document.
    SlabAllocator(void* headerMemoryStart, size_t headerMemorySize,
                  void* slabMemoryStart, size_t slabMemorySize);
    ...
    FOLLY_ALWAYS_INLINE SlabHeader* getSlabHeader(
        ...
        if (LIKELY(isValidSlab(slab))) {
            const auto slabIndex =
                static_cast<SlabIdx>(slab - slabMemoryStart_);
    ...
    FOLLY_ALWAYS_INLINE const Slab* getSlabForMemory(
        ...
        return reinterpret_cast<const Slab*>(
            reinterpret_cast<uintptr_t>(memory) & kAddressMask);
    }
   ...
   private:
    ...
    const Slab* getSlabMemoryEnd() const noexcept {
        return reinterpret_cast<Slab*>(
            reinterpret_cast<uint8_t*>(slabMemoryStart_) + slabMemorySize_);
    }
