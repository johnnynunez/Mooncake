                            ...
                            const char *device_name)
{
    ...
    int ret = engine_->init(local_hostname, hostname_port.first.c_str(), hostname_port.second);
    if (!engine_)
        return -1;
    ...
    {
        std::string nic_priority_matrix = "{\"cpu:0\": [[\"" + std::string(device_name) + "\"], []]}";
    ...
    {
        void **current = (void **) ((char *) managed_buffer_ + i * kMaxBufferSize);
        void *next = i + 1 == kBufferCount ? nullptr : (char *) managed_buffer_ + (i + 1) * kMaxBufferSize;
        *current = next;
    }
